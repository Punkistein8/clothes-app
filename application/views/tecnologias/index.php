<h1 class="text-center mt-10 mb-5 font-sans text-5xl font-bold tracking-tight text-gray-900">Catálogo de <span class="inline-block text-teal-400">Tecnología</span></h1>
<div class="flex justify-center">
  <button type="button" class="bg-teal-600 px-5 py-3 rounded-md text-white hover:bg-teal-700 transition-all ease-in-out" data-bs-toggle="modal" data-bs-target="#modalAgregarTec">Agregar Artículo</button>
</div>

<!-- Modal Agregar -->
<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="modalAgregarTec" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-gray-800 bg-clip-padding rounded-md outline-none text-current">
      <div class="modal-header flex flex-shrink-0 items-center justify-between p-4 border-b border-gray-200 rounded-t-md">
        <h5 class="flex justify-center text-xl text-center font-medium leading-normal text-white" id="exampleModalLabel">
          Agregar un nuevo artículo tecnológico
        </h5>
  ...
  <?php foreach ($listaTecnologias as $articulo) : ?>
    <div class="flex">
      <div class="flex flex-col gap-1">
        ...
        <a href="<?php echo site_url('tecnologias/cargarDatosTecnologia') . '/' . $articulo->id_tec ?>" aria-label="Editar Artículo" class="hint--left hint--rounded hover:scale-110 transition-all ease-in-out cursor-pointer z-10 translate-x-20 translate-y-6 relative flex justify-center items-center bg-teal-600 hover:bg-teal-700 w-12 h-12 rounded-full">
        ...
        <a href="<?php echo site_url('tecnologias/eliminarTecnologia') . '/' . $articulo->id_tec ?>" aria-label="Borrar Artículo" class="hint--left hint--rounded hover:scale-110 transition-all ease-in-out cursor-pointer z-10 translate-x-20 translate-y-6 relative flex justify-center items-center bg-red-600 hover:bg-red-700 w-12 h-12 rounded-full">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute w-6 h-6 text-white">
      ...
      <div class="flex justify-center">
        <div id="card" class="flex-alcohol max-w-xs rounded-lg m-10 bg-slate-100 shadow-lg hover:scale-105 transition-all ease-in-out cursor-pointer">
          ...
          <div class="p-6 flex flex-col justify-start">
            <h5 id="nombre-prenda" class="text-gray-900 text-xl font-medium mb-2"><?php echo $articulo->nombre_tec ?></h5>
            <p id="precio" class="text-gray-600 text-xl font-bold"><span class="font-normal text-md">Marca:</span> <?php echo $articulo->marca_tec ?></p>
            <p id="precio" class="text-gray-600 text-xl font-bold">$<?php echo $articulo->precio_tec ?></p>
          </div>
        </div>
      </div>
    </div>
