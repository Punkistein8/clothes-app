<?php
require "./controllers/Hombres.php";
require_once "./templates/header.php";
$response = fetchBDHombres();
?>
...
<div class="flex flex-wrap justify-center">
  ...
  <div id="card" class="flex-row max-w-xs rounded-lg m-10 bg-slate-100 shadow-lg hover:scale-105 transition-all ease-in-out cursor-pointer">
    <img class="w-full rounded-t-xl max-h-fit" src="' . $row['foto_ph'] . '" alt="" />
    <div class="p-6 flex flex-col justify-start">
      <h5 id="nombre-prenda" class="text-gray-900 text-xl font-medium mb-2">' . $row['nombre_ph'] . '</h5>
      <p class="text-gray-700 text-base mb-4">
      ' . $row['descripcion_ph'] . '
      </p>
      <p id="precio" class="text-gray-600 text-xl font-bold">$' . $row['precio_ph'] . '</p>
    </div>
  </div>
</div>');
  }
